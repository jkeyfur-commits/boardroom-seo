...
/**
 * One retry on a transient failure before anything is recorded as broken.
 *
 * NOT DEFENSIVE PADDING. On 2026-08-16 a run reported two pages down and a
 * redirect broken; all three served 200 on the very next request, twice. That
 * is three critical alarms — in an email whose entire value rests on silence
 * meaning "fine" — caused by nothing at all.
 *
 * Retried: a thrown request (status 0), 429, and 5xx. Those are the shapes a
 * blip takes. A 404 is NOT retried: it is a real answer, arrived at quickly,
 * and retrying it would only slow the crawl down to confirm what it was told.
 *
 * The delay is deliberate. An immediate retry hits the same overloaded edge
 * and fails the same way, which produces a slower crawl and identical alarms.
 */
const transient = (s) => s === 0 || s === 429 || s >= 500;

async function get(url, opts = {}) {
  const first = await getOnce(url, opts);
  if (!transient(first.status)) return first;
  await new Promise((r) => setTimeout(r, 1200));
  const second = await getOnce(url, opts);
  // Report the retry's result either way: if it also failed, that is now
  // evidence rather than a coin flip, and the finding deserves to fire.
  return { ...second, retried: true };
}

async function getOnce(url, { redirect = 'follow', timeoutMs = 20000 } = {}) {
...
export async function crawlSite(site, { readFile } = {}) {
  ...
  const hostChecks = await pool([site.domain, ...(site.extra_hosts || [])], 4, async (host) => {
    ...
    // WHERE it ended up, not just whether it answered. An alias host that
    // serves 200 in its own right is a duplicate of the site competing with
    // it, and "reachable" is exactly the wrong question to ask about one.
    return {
      host,
      status: r.status,
      ms: r.ms,
