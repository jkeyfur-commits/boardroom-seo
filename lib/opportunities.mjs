 ...
 */
export function linkingOpportunities(pages, targets, bodyText) {
  const out = [];
  for (const t of targets) {
    for (const p of pages) {
      if (p.url === t.url || p.http_status !== 200) continue;
      const text = (bodyText.get(p.url) || '').toLowerCase();
      if (!text) continue;
      // Already links there — nothing to suggest.
      if (text.includes(t.url.replace(/^https?:\/\/[^/]+/, ''))) continue;
...
const isUtility = (url) =>
  ...
  /\/(thank-you|lead-problem|404|privacy|terms|sitemap)/.test(url);
