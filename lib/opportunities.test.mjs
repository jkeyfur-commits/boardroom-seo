...
test('legal and debtor-facing pages get no link suggestions', () => {
  // A privacy notice does not need a route into the funnel, and the
  // debtor-facing payment/dispute pages are not ranking targets. This rule
  // was the only one of the four not applying the utility filter.
  const text = new Map();
  const pages = ['/privacy', '/terms', '/dispute-an-account', '/make-a-payment'].map((p) => {
    text.set(u(p), 'Phoenix is mentioned here.');
    return page({ url: u(p) });
  });
  assert.deepEqual(linkingOpportunities(pages, targets, text), []);
});

test('the debtor-facing pages are not called thin either', () => {
  const pages = [page({ url: u('/make-a-payment'), word_count: 140 })];
  assert.deepEqual(metadataWork(pages), []);
});

// ---------------------------------------------------------- search data

const qp = (query, page, over = {}) => ({
  keys: [query, u(page)],
  impressions: 100, clicks: 0, position: 8, ...over,
});

test('a query ranking 5-20 with real volume is striking distance', () => {
  const out = searchOpportunities({ byQueryAndPage: [qp('debt collection phoenix', '/services')] });
  const hit = out.find((o) => o.kind === 'striking_distance');
  assert.ok(hit);
  assert.equal(hit.impact, 'high');
  assert.match(hit.title, /debt collection phoenix/);
});

test('already on page one is not an opportunity', () => {
  const out = searchOpportunities({ byQueryAndPage: [qp('aro', '/', { position: 2.1 })] });
  assert.deepEqual(out.filter((o) => o.kind === 'striking_distance'), []);
});

test('ranking beyond page two is not "a small push"', () => {
  const out = searchOpportunities({ byQueryAndPage: [qp('collections', '/', { position: 44 })] });
  assert.deepEqual(out.filter((o) => o.kind === 'striking_distance'), []);
});

test('a handful of impressions is noise, not a finding', () => {
  // ARO's entire site draws ~700 impressions a month. Acting on a query with
  // four of them wastes a slot on a list capped at twelve.
  const out = searchOpportunities({ byQueryAndPage: [qp('x', '/', { impressions: 4 })] });
  assert.deepEqual(out, []);
});

test('impressions with zero clicks is a title problem', () => {
  const out = searchOpportunities({ byQueryAndPage: [qp('q', '/about', { position: 3 })] });
  const hit = out.find((o) => o.kind === 'impressions_no_clicks');
  assert.ok(hit);
  assert.match(hit.fix, /title and description/);
});

test('a page with clicks is not reported as unclicked', () => {
  const out = searchOpportunities({ byQueryAndPage: [qp('q', '/about', { position: 3, clicks: 5 })] });
  assert.deepEqual(out.filter((o) => o.kind === 'impressions_no_clicks'), []);
});

test('impressions are summed across queries for one page', () => {
  const out = searchOpportunities({
    byQueryAndPage: [
      qp('a', '/about', { impressions: 12, position: 3 }),
      qp('b', '/about', { impressions: 15, position: 3 }),
    ],
  });
  const hit = out.find((o) => o.kind === 'impressions_no_clicks');
  assert.ok(hit, 'neither query clears 20 alone, together they do');
  assert.match(hit.title, /27 times/);
});

test('a real decline is flagged with both numbers', () => {
  const out = searchOpportunities({
    byQueryAndPage: [qp('q', '/results', { impressions: 30, clicks: 1, position: 3 })],
    pagesPrior: [{ keys: [u('/results')], impressions: 100 }],
  });
  const hit = out.find((o) => o.kind === 'decaying');
  assert.ok(hit);
  assert.match(hit.title, /70%/);
  assert.match(hit.title, /100 → 30/);
});

test('a page with no prior data is not reported as collapsed', () => {
  // ARO was absent from search for six months. Against an empty prior window
  // every page looks like it fell 100%, which is an artefact, not a decline.
  const out = searchOpportunities({
    byQueryAndPage: [qp('q', '/new', { impressions: 30, clicks: 1, position: 3 })],
    pagesPrior: [],
  });
  assert.deepEqual(out.filter((o) => o.kind === 'decaying'), []);
});

test('a small dip is not decay', () => {
  const out = searchOpportunities({
    byQueryAndPage: [qp('q', '/results', { impressions: 90, clicks: 1, position: 3 })],
    pagesPrior: [{ keys: [u('/results')], impressions: 100 }],
  });
  assert.deepEqual(out.filter((o) => o.kind === 'decaying'), []);
});

test('no search data at all returns nothing, and does not throw', () => {
  assert.deepEqual(searchOpportunities(null), []);
  assert.deepEqual(searchOpportunities({}), []);
});
