...
/**
 * Shape one crawled page into a `seo_pages` row.
 *
 * EXPORTED BECAUSE THERE ARE TWO WRITERS. worker.mjs cannot use savePages —
 * it runs on Cloudflare where credentials arrive as an `env` binding rather
 * than process.env, so it has its own transport. It had its own row-shaping
 * too, which is how the `_bodyLinks` bug survived being fixed here: one
 * defect, two copies, and only the copy I was looking at got repaired.
 *
 * Transport can differ. The shape of a row must not.
 */
export function pageRow(p, { runId, company, source = SOURCE, observedAt }) {
  const row = { run_id: runId, company, source, observed_at: observedAt };
...
export async function savePages(runId, snap) {
  ...
  const rows = snap.pages.map((p) =>
    pageRow(p, { runId, company: snap.company, observedAt: snap.observed_at })
  );
